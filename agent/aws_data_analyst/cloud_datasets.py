...
class CloudDatasetLoader:
    ...
    def __load_metadata(self):
        for namespace_uri in S3.ls(S3_DATASETS_METADATA):
            namespace = namespace_uri.split('/')[-1]
            for dataset_uri in S3.ls(namespace_uri):
                dataset_id = dataset_uri.split('/')[-1]
                self.load_metadata(f"{namespace}.{dataset_id}")
    ...
    def load_metadata(self, dataset_id):
        ...
        metadata_uri = f"{S3_DATASETS_METADATA}{namespace}/{ds_id}/dataset.json"
        try:
...
class CloudQueryHandler:
    ...
    def query_dataset(self, dataset_id, dimension_filters=None):
